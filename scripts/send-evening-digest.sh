#!/usr/bin/env bash
# Sends evening digest to farm owner channels.
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ -n "${CRON_SECRET:-}" ]; then
  ...
  curl -sf -X POST "$API_URL/api/alerts/evening-digest" \
...
curl -sf -b "$COOKIE_JAR" -X POST "$API_URL/api/alerts/evening-digest" \
  -H "Content-Type: application/json" \
  -H "X-CSRF-Token: $CSRF" \
  -d '{}'
