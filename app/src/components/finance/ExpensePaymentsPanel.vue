...
<template>
  <section class="my-5 rounded-2xl border border-slate-700 bg-slate-900 p-5" data-testid="expense-payments-panel">
    <div class="flex justify-between gap-4"><h3 class="font-bold text-white">{{ t('financeTracking.payments') }} · {{ expense.description }}</h3><button type="button" class="text-slate-300" @click="emit('close')">{{ t('finance.cancel') }}</button></div>
    <p class="mt-2 text-slate-300">{{ t(`financeTracking.${expense.paymentStatus ?? 'unpaid'}`) }} · {{ t('financeTracking.balance') }}: {{ money(balance) }}</p>
    ...
    <form v-if="canWrite && expense.approvalStatus === 'approved' && balance > 0" class="mt-5 grid gap-3 sm:grid-cols-3" @submit.prevent="record">
      ...
      <button :disabled="busy || !expense.paymentDueDate" class="rounded bg-farm-green p-3 font-bold text-white disabled:opacity-50">{{ t('financeTracking.recordPayment') }}</button>
    </form>
    <ul class="mt-5 space-y-2"><li v-for="payment in payments" :key="payment.id" class="rounded bg-slate-800 p-3 text-slate-200">{{ payment.paidOn }} · {{ money(payment.amount, payment.currency) }} · {{ payment.reference }}</li></ul>
    <p v-if="!payments.length" class="mt-4 text-sm text-slate-400">{{ t('financeTracking.noPayments') }}</p>
  </section>
</template>
