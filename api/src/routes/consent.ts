...
import { computeConsentStatus, consentPostSchema } from '../lib/consent.js'

export const consentRoutes = new Hono<{ Variables: AppVariables }>()

consentRoutes.use('*', authMiddleware)

consentRoutes.post('/', zValidator('json', consentPostSchema), async (c) => {
...
consentRoutes.get('/status', async (c) => {
  ...
  return c.json(computeConsentStatus(rows, user.id))
})
