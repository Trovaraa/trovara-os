...
export function parseCropCycleIntent(text: string): {
  ...
  const match = trimmed.match(
    /^(?:create\s+)?crop(?:\s+cycle)?\s*[:\-–]?\s*(.+?)\s+type\s*=\s*(\S+)\s+planted\s*=\s*(\d{4}-\d{2}-\d{2})(?:\s+harvest\s*=\s*(\d{4}-\d{2}-\d{2}))?(?:\s+yield\s*=\s*(\d+))?$/i,
  )
...
/** Livestock: Broiler A species=broiler heads=200 [plot=Block 2] [acquired=YYYY-MM-DD] */
export function parseLivestockBatchIntent(text: string): {
  ...
  const trimmed = text.trim()
  const match = trimmed.match(
    /^(?:create\s+)?(?:livestock|batch)\s*[:\-–]?\s*(.+?)\s+species\s*=\s*(\S+)\s+heads?\s*=\s*(\d+)(?:\s+plot\s*=\s*(.+?))?(?:\s+acquired\s*=\s*(\d{4}-\d{2}-\d{2}))?\s*$/i,
  )
  if (!match) return null
  return {
    name: match[1].trim(),
